...
void saveMacroToPath(std::filesystem::path path) {
...
bool loadMacroFromPath(std::filesystem::path path) {
...
// ===== Save Layer (xdBot 스타일) =====
class SaveMacroLayer : public CCLayer {
public:
    ...
    static void open(CCNode* parent) {
        if (g.macro.inputs.empty()) {
            FLAlertLayer::create("dim5lBOT", "No macro to save!", "OK")->show();
            return;
        }
        if (!std::filesystem::exists(getTempPath())) {
            FLAlertLayer::create("dim5lBOT", "Record first!", "OK")->show();
            return;
        }
        parent->addChild(SaveMacroLayer::create(), 200);
    }
    ...
    bool init() {
        ...
        title->setPosition(winSize.width / 2, winSize.height / 2 + 62);
        title->setScale(0.7f);
        addChild(title);

        nameInput = TextInput::create(220, "Macro name...");
        nameInput->setPosition(winSize / 2);
        addChild(nameInput);

        auto saveBtn = CCMenuItemSpriteExtra::create(
            ButtonSprite::create("Save", "goldFont.fnt", "GJ_button_01.png"),
            this, menu_selector(SaveMacroLayer::onSave));
        auto cancelBtn = CCMenuItemSpriteExtra::create(
            ButtonSprite::create("Cancel", "goldFont.fnt", "GJ_button_06.png"),
            this, menu_selector(SaveMacroLayer::on
...
public:
    ...
    bool init() {
        ...
        auto cancelBtn = CCMenuItemSpriteExtra::create(
            ButtonSprite::create("Cancel", "goldFont.fnt", "GJ_button_06.png"),
            this, menu_selector(SaveMacroLayer::onCancel));

        auto menu = CCMenu::create(saveBtn, cancelBtn, nullptr);
        menu->alignItemsHorizontallyWithPadding(10);
        menu->setPosition(winSize.width / 2, winSize.height / 2 - 52);
        addChild(menu);
        return true;
    }
    ...
    void onSave(CCObject*) {
        ...
        if (name.empty()) {
            FLAlertLayer::create("dim5lBOT", "Give a name to the macro.", "OK")->show();
            return;
        }
...
// ===== Load Layer =====
class LoadMacroLayer : public CCLayer {
public:
    ...
    bool init() {
        ...
        for (auto& entry : std::filesystem::directory_iterator(dir)) {
            ...
            auto btn = CCMenuItemSpriteExtra::create(
                ...
                this, menu_selector(LoadMacroLayer::onLoadFile));
        ...
        auto closeBtn = CCMenuItemSpriteExtra::create(
            ...
            this, menu_selector(LoadMacroLayer::onClose));
        auto closeMenu = CCMenu::create(closeBtn, nullptr);
        closeMenu->setPosition(winSize.width / 2, winSize.height / 2 - 115);
        addChild(closeMenu);
        return true;
    }
    ...
    void onLoadFile(CCObject* sender) {
        ...
        if (loadMacroFromPath(pathStr)) {
            Notification::create(
                fmt::format("Loaded {} inputs!", g.macro.inputs.size()),
...
public:
    ...
    void onStop(CCObject*) {
        ...
        if (wasRecording && !g.macro.inputs.empty()) {
            saveMacroToPath(getTempPath());
            Notification::create(
                fmt::format("Stopped! {} inputs recorded.", g.macro.inputs.size()),
                NotificationIcon::Success)->show();
        } else {
    ...
    void onSave(CCObject*) {
        SaveMacroLayer::open(getParent() ? getParent() : this);
    }

    void onLoad(CCObject*) {
        if (auto parent = getParent())
            parent->addChild(LoadMacroLayer::create(), 200);
    }
